...
function doGet(e) {
  try {
    ...
    return json_({ ok: true, profiles: profiles, agents: agents, stories: stories });
  } catch (err) {
...
function saveStory_(data) {
  const session = requireSession_(data.token);
  if (session.role !== 'admin') throw new Error('Only admin can manage success stories');

  const sheet = getSheet_(STORY_SHEET, STORY_HEADERS);
  const existing = data.id ? tryFindById_(sheet, data.id) : null;
  const story = {};
  STORY_HEADERS.forEach(header => story[header] = data[header] || '');
  story.id = existing ? existing.item.id : (story.id || ('ST' + Date.now().toString().slice(-8)));
  story.timestamp = existing ? (existing.item.timestamp || new Date()) : new Date();
  story.status = story.status || 'draft';
  story.consent = story.consent || 'yes';

  if (existing) {
    updateRow_(sheet, existing.row, STORY_HEADERS, story);
  } else {
    appendObject_(sheet, STORY_HEADERS, story);
  }
  return json_({ ok: true, story: story });
}

function deleteStory_(data) {
  const session = requireSession_(data.token);
  if (session.role !== 'admin') throw new Error('Only admin can delete success stories');
  const sheet = getSheet_(STORY_SHEET, STORY_HEADERS);
...
function editProfile_(data) {
  ...
  if (session.role === 'agent' && !canAgentManageProfile_(session, existing.item)) {
    throw new Error('Agent can edit only their own client data');
  }
...
function setProfileStatus_(data) {
  const session = requireSession_(data.token);
  if (session.role !== 'admin') throw new Error('Only admin can approve client profiles');
...
function savePayment_(data) {
  ...
  if (session.role === 'agent' && !canAgentManageProfile_(session, found.item)) {
    throw new Error('Agent can add payment only for their own client data');
  }
...
function canAgentManageProfile_(session, profile) {
  ...
  return profileAgentId === String(session.agentId || '').trim();
}
